...
const PrintMaterial: React.FC<Props> = ({ show, onClose }) => {
    const contentRef = useRef(null);
    const handlePrint = useReactToPrint({ contentRef });
    const { processing, setProcessing, setBackdrop } = useProcessing();
    const [data, setData] = useState<Array<RAWMATERIALS> | null>(null);
    ...
    useEffect(() => {
        if (show) {
            ...
            setProcessing(true);
            window.axios
                .get<Array<RAWMATERIALS>>(
                    route("manage.inventory.raw.json", {
                        _query: {
                            paginate: true,
                        },
                    })
            ...
            {!processing && (
                <div className="h-fit w-full flex flex-col relative">
                    ...
                    <div className="mx-auto w-fit h-fit pt-5 pb-8">
                        <div
                            ref={contentRef}
                            className="bg-white w-[8.5in] print:pb-0 pb-10"
                        >
                            <style>
                                {`
                                @media print {
                            ...
                            <div className="grid grid-cols-[5rem,1fr,5rem] mx-auto w-fit gap-3 pt-10 print:pt-0">
                                ...
                                <div className="text-center self-center">
                                    <div className="uppercase">
                                        Ellie's Cheesy Pizza
                                    </div>
                                    <div className="">
                                        Villarosa, New Visayas, Panabo City
                                    </div>
                                </div>
                                <div></div>
                            </div>
                                ...
                                <table className="table border border-black [&>thead_th]:border-black [&>tbody_td]:border-black [&>tbody_tr]:border-black mx-auto w-fit">
                                    <thead className="">
                                        <tr className="divide-x border-black text-black">
                                            ...
                                            <th className="w-28 px-1 text-center">
                                                Re-Stock Level
                                            </th>
                                            <th className="w-28 px-1 text-center">
                                                Date Added
                                        ...
                                        {data &&
                                            data.map((product, index) => (
                                                <tr
                                                    ...
                                                    className="divide-x"
                                                >
                                                    <td className="w-60 p-1">
                                                        {product.name}
                                                    </td>
                                                    <td className="w-20 p-1 text-center">
                                                        {product.stock_in ?? 0}
                                                    </td>
                                                    <td className="w-28 p-1 text-center">
